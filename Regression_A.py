...
df_encoded = pd.get_dummies(df, columns=['Car Make', 'Country'], drop_first=True, dtype=int)
...
attributeNames = ['Offset', 'Car Make', 'Year', 'Horsepower', 'Torque (lb-ft)', '0-60 MPH Time (seconds)', 'Country']
N, M = X.shape
...
lambdas = np.array([1e-8, 10**-3, 10**-2, 10**-1, 10**1, 10**2, 10**3, 10**4, 10**5], dtype=float)
...
for train_index, test_index in CV.split(X,y):
    ...
    w_noreg[:,k] = np.linalg.solve(XtX + AvoidCrash,Xty).squeeze() # TODO: ADD HERE SMALL REGULARIZATION TERM FOR NUMERICAL STABILITY
